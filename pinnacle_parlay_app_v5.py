...
st.title("🎯 Parlay FAIR Value (no‑vig) — Full Game Filter (v5)")
st.caption("Now filters to full-game moneylines (period 0). Toggle debug to inspect the chosen market.")
...
if st.button("Calculate FAIR value"):
    ...
    for r in results:
        debug_block = ""
        if show_debug and r.get("_market_debug"):
            md = r["_market_debug"]
            pnum = md.get("period",{}).get("number", md.get("period",{}).get("id"))
            debug_block = f"  \nDebug: type={md.get('type')} status={md.get('status')} period={pnum}"
        st.markdown(
