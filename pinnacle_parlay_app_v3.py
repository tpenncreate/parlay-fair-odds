...
def build_team_index(matchups):
    ...
    for m in matchups or []:
        ...
        if len(parts) < 2:
            continue
        # Try to identify home/away; fall back gracefully
...
st.title("🎯 Parlay FAIR Value (no‑vig) — Picker Mode (v3)")
st.caption("If you see 'no matchups', click **Force refresh** below or try a different league.")
...
if not all_team_names:
    st.warning("No current matchups found for this league (or feed blocked). Try **Force refresh**, a different league, or later today.")
    st.stop()
