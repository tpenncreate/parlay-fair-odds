...
DEFAULT_API_BASE = "https://api.opticodds.com/api/v3"
...
def list_active_fixtures(api_base: str, api_key: str, league_id: str):
    headers = make_headers(api_key)
    data, err = safe_get(f"{api_base}/fixtures/active", headers, params={"league": league_id})
    return data, err

@st.cache_data(ttl=90, show_spinner=False)
def get_fixture_odds_moneyline(api_base: str, api_key: str, fixture_id: str, sportsbook: str = "Pinnacle"):
    headers = make_headers(api_key)
...
# ---------------- UI ----------------
st.set_page_config(page_title="MLB Parlay — OddsJam API (No Leagues scope)", page_icon="🔐")
st.title("⚾ MLB Parlay — Fair Odds + Kelly (OddsJam API)")

with st.expander("Setup", expanded=True):
    api_key = st.text_input("API key", type="password", help="Paste your OddsJam/OpticOdds API key here.")
    api_base = st.text_input("API base", value=DEFAULT_API_BASE, help="Usually https://api.opticodds.com/api/v3")
    league_id = st.text_input("League id", value="mlb", help="Try 'mlb'. If your account uses different ids, enter it here.")
    col1, col2 = st.columns([1,1])
    with col1:
        if st.button("Test fixtures"):
            if not api_key:
                st.error("Enter your API key first.")
            else:
                data, err = list_active_fixtures(api_base, api_key, league_id)
                if err:
                    st.error(f"Fixtures error: {err}")
                else:
                    st.success("✅ Fixtures accessible")
                    st.json((data or {}) if isinstance(data, dict) else data)

if not api_key:
    st.warning("Enter your API key in Setup to continue.")
    st.stop()

# Pull fixtures
fdata, ferr = list_active_fixtures(api_base, api_key, league_id)
...
if st.button("Calculate FAIR value & Kelly"):
    ...
    for pick in picks:
        ...
        prices, perr = get_fixture_odds_moneyline(api_base, api_key, fid, sportsbook)
