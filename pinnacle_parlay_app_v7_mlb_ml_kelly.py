...
def parse_boosted_odds(s: str):
    """
    Accepts '+450', '-120', '5.50', '2.1' and returns decimal odds.
    """
    s = normalize(str(s))
    if not s:
        return None
    try:
        # American if starts with +/-, or if it's an int >= 100 or <= -100
        if s.startswith(('+','-')) or (s.isdigit() and abs(int(s)) >= 100):
            return american_to_decimal(float(s))
        # Otherwise treat as decimal
        val = float(s)
        # Guard: if user typed '450' intending +450, treat >= 100 as American
        if val >= 100:
...
@lru_cache(maxsize=1)
def guest_key() -> str:
    cfg = backoff_get("https://www.pinnacle.com/config/app.json", headers={"User-Agent":"Mozilla/5.0"}, timeout=15).json()
    try:
...
@st.cache_data(ttl=30, show_spinner=False)
def get_all_markets(league_id: int):
    r = backoff_get(f"https://guest.api.arcadia.pinnacle.com/0.1/leagues/{league_id}/markets", headers=make_headers())
    return r.json()
...
def get_ml_prices_any_source(league_id, matchup_id):
    ...
    cand = collect_ml_candidates(lm, matchup_id)
    if cand:
    ...
    # 2) league all
    lm_all = get_all_markets(league_id)
    cand = collect_ml_candidates(lm_all, matchup_id)
    if cand:
        chosen = choose_candidate(cand)
        if chosen:
            prices = {}
    ...
    # 3) per-matchup markets (last resort)
    mm = get_matchup_markets(matchup_id)
    cand = collect_ml_candidates(mm, matchup_id)
    if cand:
        chosen = choose_candidate(cand)
        if chosen:
            prices = {}
...
def compute_leg(selection, league_id):
...
def kelly_fraction(p: float, dec_odds: float):
    """Return full Kelly fraction for decimal odds and win prob p."""
    if p is None or dec_odds is None or dec_odds <= 1.0:
        return None
    b = dec_odds - 1.0
    q = 1.0 - p
    denom = b
    num = b * p - q  # Kelly formula (bp - q) / b
    f = num / denom
    return f
...
# Resolve MLB league id automatically
sports = get_sports()
baseball = next((s for s in sports if "baseball" in (s.get("name","").lower())), None)
if not baseball:
    st.error("Couldn't find Baseball in sports list.")
    st.stop()
leagues = get_leagues(baseball.get("id"))
mlb = None
for lg in leagues:
    ...
    if "mlb" in nm or "major league baseball" in nm:
        ...
        break
if not mlb:
    st.error("Couldn't find MLB league.")
    st.stop()

league_id = mlb.get("id")
...
if not all_team_names:
    st.warning("No MLB teams listed right now. Try again later.")
    st.stop()

selected_teams = st.multiselect("Choose MLB teams for your parlay (moneyline)", options=all_team_names, default=[])
...
if st.button("Calculate FAIR value & Kelly"):
    ...
    for name in selected_teams:
        ...
        res = compute_leg(selection, league_id)
        if res:
            results.append(res)
    ...
    if not results:
        st.error("Couldn't locate moneyline prices for selected teams.")
        st.stop()
    ...
    # Expected value per $1 staked at offered odds
    b = offered_dec - 1.0
